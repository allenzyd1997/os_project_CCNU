...
from algorithms import algorithm_memory_apply_best_adapt, algorithm_memory_recycle,  algorithm_memory_apply_first_adapt,  algorithm_memory_apply_worst_adapt
from job import Job 
...
class System_2(object):
	...
	def __init__(self, memory_size = 1024 * 8):
		...
		# ************************************************************* 
		# ALGORITHM AREA
		self.apply_memory_algorithm = algorithm_memory_apply_first_adapt
		...
		self.events 				= [ 
										Event(name = "event1", semaphore = 1),
										Event(name = "event2", semaphore = 1),
										Event(name = "event3", semaphore = 1),
										Event(name = "event4", semaphore = 1),
										Event(name = "event5", semaphore = 1)
										]
		self.current_event 			= []
		...
		# ************************************************************* 
		# FILE INIT
		with open("suspend_list.csv", "w") as f:
			f.write("pid,priority,event_name,happen_time,running_time\n")
			f.close()
	...
	def event_happen(self):
		...
		self.event_sustain = YidanTime(random.randint(3, 5))
		self.show_event()
	...
	def menu(self):
		...
		while(1):
			# try:
				...
				print("6. add running job to hungup")
				print("7. event happen")
				print("8. io")
				print("9. dispatch job")
				print("10. close menu")
				print("11. add script data")
				print("12. event all happen")
				print("13. de-suspend the process, activate and add to ready list")
				print("14. show the list")
				...
				if choice == 1 :
					...
					self.current_event = [	Event(name = "event1", semaphore = 2),
											Event(name = "event2", semaphore = 1),
					...
					print("Please Input a PID FOR AWAKING")
					pid = int(input())
					for event_list in self.blocked_list:
						for process in self.blocked_list[event_list]:
							if process.pcb.pid == pid:
								self.ready_list[event_list].append(process)
								self.blocked_list[event_list].remove(process)
								break
	...
	def show_list(self):
		print("{:>50}".format("Running process"))
		if self.running_process == None:
			print("None")
		else:
			print(self.running_process.pcb.pid)
	...
	def add_block_to_ready(self):
		for event_name in [event.name for event in self.current_event]:
			self.ready_list[event_name] += self.blocked_list[event_name]
			self.blocked_list[event_name] = []
		return 
