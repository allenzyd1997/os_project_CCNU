def algorithm_memory_apply_best_adapt(empty_memory_list, apply_memory_size, threshold):
	...
	empty_memory_list.sort(key = lambda x: len(x))
	result,lower_bound, upper_bound = False,None, None 
	if empty_memory_list == []:
		...
		# can finish the job
		lower_bound, upper_bound = None, None
		for memory in empty_memory_list:
			if len(memory) >= apply_memory_size:
...
def algorithm_memory_apply_worst_adapt(empty_memory_list, apply_memory_size, threshold):
	# This is the algorithm used for helping the os finish the memory allocation task
	# Take the best adapt algorithm, which is, choose the biggest memory block in the list and allocate this bolck
	# return value: 
	# Bool Value, whether operation is successful or not.
...
def algorithm_memory_apply_first_adapt(empty_memory_list, apply_memory_size, threshold):
	# This is the algorithm used for helping the os finish the memory allocation task
	# Take the best adapt algorithm, which is, choose the biggest memory block in the list and allocate this bolck
	# return value: 
	# Bool Value, whether operation is successful or not.
...
def algorithm_memory_recycle(empty_memory_list, allocated_memory_list, recycled_memory):
	...
	allocated_memory_list.remove(recycled_memory)
